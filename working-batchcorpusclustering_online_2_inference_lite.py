...
CLUSTER_BEAM_SIZE = 4 #10 #@param [1, 2, 5, 10, 100] {"type":"raw"}
SENTENCE_BEAM_SIZE = 4 #10 #@param [1, 2, 5, 10, 100] {"type": "raw"}

NUM_CLUSTERS = 32 #1000 #@param [1, 100, 200, 500, 1000, 1024] {"type": "raw"}
...
EMBEDDER_BATCH_SIZE = 64 # 32768 #@param [32, 64, 128, 256, 512, 1024, 2048, 4096, 16384, 32768] {"type": "raw"}
BATCH_REPORT_FREQ_FACTOR = 1 # 10 #@param [1, 10, 100, 1000] {"type": "raw"}
...
MAX_TMP_EMBEDDING_FILES_TO_STORE = 3 #10 #@param [2, 5, 10, 20] {"type": "raw"}
...
def create_tmp_embeddings(embedder):
    ...
    for idx_record, record in enumerate(all_records):
        ...
        for i in range(0, len(flattened_data), EMBEDDER_BATCH_SIZE):
            ...
            if not initialized_x:
                X = crt_embedding
                initialized_x = True
                print('>>> Not initialized, correct, X shape is {}'.format(X.shape))
            else:
                print('>>> Initialized, stacking over previous X shape: {}'.format(X.shape))
                X = torch.vstack((X, crt_embedding))
                print('>>> Newly obtained X shape: {}'.format(X.shape))

            # if X gets too large, dump its embedding contents to temporary embedding files
            if X.shape[0] > 100: # MAX_KMEANS_FITTING_SIZE:# 100000:
    ...
    print('Remaining bin size: {} | processed kmeans: {} | num loaded: {} | num embedded: {} | kmeans x: {}'.format(len(bins), processed_examples, num_loaded_examples, num_embedded_examples, kmeans_x_examples))
...
def compressed_construct_bins():
    ...
    for bin_ in range(NUM_CLUSTERS):
        ...
        print('Assembling bin {}/{}'.format(bin_, NUM_CLUSTERS))
...
def inference_multiple_text_records(test_sentence, embedder, kmeans, bins=None, ipca_transformer=None, cluster_beam_size=5, sentence_beam_size=5):
    data_text_, sentence_idx_boundaries = get_text_data_full() # get_text_data()
    start = time.time()
    target_idx = retrieve_similar_sentences(test_sentence, embedder, kmeans, bins=bins, ipca_transformer=ipca_transformer, cluster_beam_size=cluster_beam_size, sentence_beam_size=sentence_beam_size)
    end = time.time()
    print('Sentence retrieval took {} seconds\n'.format(end - start))
